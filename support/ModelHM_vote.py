...
from support.DatasetHM_DistBERT import HatefulMemesDataset
from support.Concat_vote import LanguageAndVisionConcat
...
class HatefulMemesModel(pl.LightningModule):
    ...
    def forward(self, image, input_ids, attention_mask, label=None):
        return self.model(image, input_ids, attention_mask, label)

    def training_step(self, batch, batch_nb):
        predV, predT, lossV, lossT = self.forward( 
            image=batch["image"],
            input_ids=batch["input_ids"],
            attention_mask=batch["attention_mask"],
            label=batch["label"]
        )
        lossT_d = lossT.clone().to('cpu').detach()
        lossV_d = lossV.clone().to('cpu').detach()
        predV = predV.to('cpu').detach()
        predT = predT.to('cpu').detach()

        preds = torch.cat((predV, predT), dim=1)
        max, indices = torch.max(preds, dim=1)
        strongPreds = torch.where(indices < 2,  torch.transpose(predV, 0, 1), torch.transpose(predT, 0, 1))
        strongPreds = torch.transpose(strongPreds, 0, 1)
        #strongPreds = torch.where(torch.transpose(indices, 0, 1) < 2, predV, predT)
        labels = batch["label"].to('cpu').detach()
        acc = strongPreds[torch.arange(predV.shape[0]), labels]
        ave_acc = acc.mean()
        #accV = predV[torch.arange(predV.shape[0]), labels]
        #aveV_acc = accV.mean()
        #accT = predT[torch.arange(predT.shape[0]), labels]
        #aveT_acc = accT.mean()
        metric = BinaryAUROC(thresholds=5)
        acc_auroc = metric(acc, labels)
        
        #self.log("lossV", lossV, on_step=False, on_epoch=True, prog_bar=True, logger=True)
        return {"loss": lossV, "lossT": lossT_d, "lossV": lossV_d, "ave_acc": ave_acc, "acc_auroc": acc_auroc}#, "aveT_acc": aveT_acc, "aveV_acc": aveV_acc}

    def validation_step(self, batch, batch_nb):
        predV, predT, lossV, lossT = self.eval().forward(
            image=batch["image"],
            input_ids=batch["input_ids"],
            attention_mask=batch["attention_mask"],
            label=batch["label"]
        ...
        return {"batch_val_lossV": lossV_d, "batch_val_lossT": lossT_d, "ave_acc": ave_acc, "acc_auroc": acc_auroc}#, "aveT_acc": aveT_acc, "aveV_acc": aveV_acc}

    def validation_epoch_end(self, outputs):
        avg_lossV = torch.stack(
            tuple(
                output["batch_val_lossV"] 
                for output in outputs
            )
        ).mean()
        avg_lossT = torch.stack(
            tuple(
                output["batch_val_lossT"] 
                for output in outputs
            )
        ).mean()
        avg_acc = torch.stack(
        ...
        self.val_lossV.append(avg_lossV)
        self.val_lossT.append(avg_lossT)
        self.val_acc.append(avg_acc)
        self.val_auroc.append(avg_auroc)

        f = open('val_tatus_epoch' + str(self.current_epoch) + '.txt', mode= 'w', encoding='utf-8')
        f.write("Image Loss:" + "\n")
        f.write(str(avg_lossV.item()) + "\n\n")
        f.write("Text Loss:" + "\n")
        f.write(str(avg_lossT.item()) + "\n\n")
        f.write("Accuracy:" + "\n")
        f.write(str(avg_acc.item()) + "\n\n")
        f.write("AUROC:" + "\n")
        f.write(str(avg_auroc.item()) + "\n\n")
        f.close()
        
        return {
            "val_lossV": avg_lossV,
            "progress_bar":{"val_lossV": avg_lossV}#, "val_acc": avg_acc, "val_auroc": avg_auroc}
        }
    ...
    def training_epoch_end(self, outputs):
        ...
        self.train_lossV.append(avg_lossV)
        self.train_lossT.append(avg_lossT)
        self.train_acc.append(avg_acc)
        self.train_auroc.append(avg_auroc)

        f = open('train_tatus_epoch' + str(self.current_epoch) + '.txt', mode= 'w', encoding='utf-8')
        f.write("Image Loss:" + "\n")
        f.write(str(avg_lossV.item()) + "\n\n")
        f.write("Text Loss:" + "\n")
        f.write(str(avg_lossT.item()) + "\n\n")
        ...
        torch.save({
            ...
            'lossT': self.train_lossT,
            'lossV': self.train_lossV
    ...
    def get_visuals(self):
        ...
        plt.plot(self.train_lossV, label='train, imageM')
        plt.plot(self.val_lossV, label='validation, imageM')
        plt.plot(self.train_lossT, label='train, imageT')
        plt.plot(self.val_lossT, label='validation, imageT')
        plt.xlabel('epoch')
        plt.ylabel('loss')
        plt.legend()
    ...
    def _build_text_transform(self):
        ...
        #)
        #f.close()
        language_transform = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')
    ...
    def _build_model(self):
        ...
        #)
        language_module = DistilBertModel.from_pretrained('distilbert-base-uncased')
        ...
        #    pretrained=True
        #)
        vision_module = torchvision.models.efficientnet_b0(
            weights='DEFAULT'
        )
        ...
        return LanguageAndVisionConcat(
            ...
            loss_fnV=torch.nn.CrossEntropyLoss(),
            loss_fnT=torch.nn.CrossEntropyLoss(),
